...
public class HexGridChunk : MonoBehaviour
{
	...
	void Awake()
	{
		...
		cells = new HexCell[HexMetrics.kChunkSizeX * HexMetrics.kChunkSizeZ];
		ShowUI(false);
	}
		...
		if(!cell.IsUnderwater)
		{
			...
			if (reversed) 
			{
				rivers.AddTriangleUV(
					...
					new Vector2(1f, 0.5f-HexMetrics.kRiverSolidUVStep), new Vector2(0f, 0.5f-HexMetrics.kRiverSolidUVStep)
				);
			}
			else 
			{
				rivers.AddTriangleUV(
					new Vector2(0.5f, 0.5f),
					new Vector2(0f, 0.5f+HexMetrics.kRiverSolidUVStep), new Vector2(1f, 0.5f+HexMetrics.kRiverSolidUVStep)
				);
			}
		}
	}
	...
	void TriangulateAdjacentToRiver ( HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e ) 
	{
		...
		if (cell.HasRiverThroughEdge(direction.Next())) 
		{
			...
			{
				center += HexMetrics.GetSolidEdgeMiddle(direction) * (HexMetrics.kInnerToOuter * 0.5f);
			}
	...
	void TriangulateRoadAdjacentToRiver ( HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e ) 
	{
		...
		else if (previousHasRiver && nextHasRiver) 
		{
			...
			Vector3 offset = HexMetrics.GetSolidEdgeMiddle(direction) * HexMetrics.kInnerToOuter;
			...
			if (!cell.IsUnderwater) 
			{
				if(!neighbor.IsUnderwater)
				{
					TriangulateRiverQuad(
						...
						0.5f+HexMetrics.kRiverSolidUVStep*2f,
						1-HexMetrics.kSolidFactor,
	...
	)
	{
		...
		for (int i = 2; i < HexMetrics.kTerraceSteps; i++)
		{
			EdgeVertices e1 = e2;
			Color c1 = c2;
	...
	void TriangulateRiverQuad ( Vector3 v1, Vector3 v2, Vector3 v3, Vector3 v4, float y1, float y2, float v, float dv, bool reversed ) 
	{
		v1.y = v2.y = y1;
		v3.y = v4.y = y2;
		...
		if (reversed) 
		{
			rivers.AddQuadUV(1f, 0f, 1-v, 1-v-dv);
		}
		else {
			rivers.AddQuadUV(0f, 1f, v, v + dv);
		}
	}
	...
	void TriangulateWaterShore ( HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center ) 
	{
		...
		{
			//计算水面跟河流的交点
			float factor = (cell.WaterSurfaceY-cell.RiverSurfaceY)/(neighbor.RiverSurfaceY - cell.RiverSurfaceY);
			factor = factor * (1-HexMetrics.kSolidFactor)*2/(2-HexMetrics.kWaterFactor-HexMetrics.kSolidFactor) 
					+ (HexMetrics.kSolidFactor-HexMetrics.kWaterFactor)/(2-HexMetrics.kWaterFactor-HexMetrics.kSolidFactor);
			TriangulateEstuary(e1, e2, cell.IncomingRiver == direction, factor);
		}
		else 
		{
	...
	void TriangulateEstuary (EdgeVertices e1, EdgeVertices e2, bool incomingRiver, float factor) 
	{
		...
		Vector3 v2 = Vector3.Lerp(e1.v3, e2.v2, factor);
		Vector3 v4 = Vector3.Lerp(e1.v3, e2.v4, factor);

		estuaries.AddQuad(e2.v1, e1.v2, v2, e1.v3);
		estuaries.AddTriangle(e1.v3, v2, v4);
		estuaries.AddQuad(e1.v3, e1.v4, v4, e2.v5);
